...
첫 갱, 5·10분 상대 정글과의 골드·레벨 차, 적정글 체류 분), 첫 코어템 시각, 귀환 횟수·시각.
...
| 첫 풀캠프 완료 | 캠프별 이벤트가 없다 | 정글 CS 가 12에 도달하는 시각을 프레임 사이 보간 |
| 귀환 횟수·시각 | 귀환 이벤트가 없다 | 상점 구매를 20초 간격으로 묶어 방문으로 보고, 데스 후 75초 안이면 부활 귀환으로 분류 |
| 첫 갱 시각 | — | 사양대로 "3분 이후 첫 킬/어시/**데스** 관여" 시각. 내 정글이 털려 죽은 것도 포함된다 |

상수는 전부 `metrics.py` 상단에 모여 있다 (`WARD_LOOKBACK_MS`, `WARD_RADIUS`,
`OBJECTIVE_RADIUS`, `FULL_CLEAR_JUNGLE_CS`, `BACK_CLUSTER_GAP_MS`, `DEATH_BACK_WINDOW_MS`).
